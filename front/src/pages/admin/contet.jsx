...
function ThemeTab({ selectedPage, onPageChange }) {
  ...
  try {
    const pageHasTheme = await checkIfPageHasTheme(selectedPage);
    ...
    if (isGlobal) {
      const globalBlocks = await contentAPI.getBlocks('__global__');
      const globalArr = globalBlocks.data?.data?.blocks ?? globalBlocks.data?.blocks ?? [];
      const hasGlobalTheme = globalArr.some(
        b => b.bloc_key === '_global_theme'
      );
