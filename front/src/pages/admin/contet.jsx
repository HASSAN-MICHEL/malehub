...
import React, { useState, useEffect, useCallback, useRef } from 'react';
>>>>>>> 93051857753f80f68366d6064e548bf0fda66389
import {
  Save, Upload, X, CheckCircle, Mail, AlertCircle, RefreshCw,
  Plus, Trash2, Edit3, Eye, EyeOff,
  Megaphone, Users, Settings, Globe, Palette,
...
// ── Configuration des blocs (inchangée)
const PAGE_BLOCKS = {
  home: [
    { key: 'hero_title', label: 'Titre Hero (H1)', type: 'text', hint: 'Ex: Work, Connect, Grow' },
    { key: 'hero_subtitle', label: 'Sous-titre Hero', type: 'textarea', hint: "Espace coworking, programme d'accompagnement..." },
...
// ... (gardez tous vos composants UI existants)
...
function ThemeTab({ selectedPage, onPageChange }) {
  ...
  const [toast, setToast] = useState(null);
  const [activeCategory, setActiveCategory] = useState('colors');
  const [isGlobal, setIsGlobal] = useState(true); // true = thème global, false = thème par page
  const scrollRef = useRef(null);
  ...
  // Clé spéciale pour le thème global
  ...
  const fetchTheme = useCallback(async () => {
    ...
    try {
      ...
      // 3. Déterminer quel thème utiliser
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isGlobal) {
        // Sauvegarder en tant que thème global
        pageSlug = '__global__';
  ...
  const switchToPage = async () => {
    setIsGlobal(false);
    // Créer une copie du thème actuel pour la page
  ...
  const hasDirty = theme.dirty === true;
  const isDefault = !theme.dirty && !theme._modified;
...
// ── CONTENT TAB CORRIGÉ (pour les images incubator) ──────────────────────────
function ContentTab({ selectedPage, onPageChange }) {
  const [contentBlocks, setContentBlocks] = useState({});
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
