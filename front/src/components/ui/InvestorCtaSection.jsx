...
export function InvestorCtaSection() {
  ...
  const title       = get('investor_title', t('investorCta.title'));
  const subtitle    = get('investor_subtitle', t('investorCta.subtitle'));
  const description = get('investor_description', t('investorCta.description'));
  const ctaText     = get('investor_cta_text', t('investorCta.cta_button'));

  const benefitsArray = getJSON('investor_benefits', Array.isArray(t('investorCta.benefits', { returnObjects: true })) ? t('investorCta.benefits', { returnObjects: true }) : []);
  const statsArray    = getJSON('investor_stats', Array.isArray(t('investorCta.stats', { returnObjects: true })) ? t('investorCta.stats', { returnObjects: true }) : []);
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <div className="max-w-4xl mx-auto">
          <div className="rounded-2xl p-8 lg:p-12 text-center relative overflow-hidden border" style={{ backgroundColor: 'var(--background)', borderColor: 'color-mix(in oklch, var(--primary) 30%, transparent)' }}>
            ...
            <h2 className="text-3xl md:text-4xl font-bold leading-tight" style={{ color: 'var(--foreground)' }}>{title}</h2>
            <p className="mt-4 text-xl font-medium" style={{ color: 'var(--primary)' }}>{subtitle}</p>
            <p className="mt-6 text-lg max-w-2xl mx-auto leading-relaxed" style={{ color: 'var(--muted-foreground)' }}>{description}</p>
