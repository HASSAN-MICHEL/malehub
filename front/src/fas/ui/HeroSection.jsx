...
//   const heroImage = get('hero_image', 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=1920&q=80');
...
//           background: 'linear-gradient(to bottom, color-mix(in oklch, var(--background) 95%, transparent), color-mix(in oklch, var(--background) 80%, transparent), var(--background))'
//         }} />
//       </div>

//       <div className="absolute top-1/4 left-0 w-32 h-px opacity-50"
//         style={{ background: 'linear-gradient(to right, transparent, var(--primary), transparent)' }} />
//       <div className="absolute bottom-1/4 right-0 w-32 h-px opacity-50"
//         style={{ background: 'linear-gradient(to right, transparent, var(--primary), transparent)' }} />
//       <div className="absolute top-1/3 right-10 lg:right-20 w-48 h-48 rounded-full border opacity-30 hidden lg:block"
...
//             {get('hero_title', 'Work, Connect, Grow')}
//           </h1>

//           <p className="mt-6 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed"
//             style={{ color: 'var(--muted-foreground)' }}>
//             {get('hero_subtitle', "Espace coworking, programme d'accompagnement, accès à un réseau d'investisseurs.")}
//           </p>

//           <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
//             <a
//               href={`https://wa.me/${waNumber}?text=Bonjour, je souhaite rejoindre l'incubateur Malea Hub`}
//               target="_blank"
//               rel="noopener noreferrer"
//               className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-4 rounded-lg text-base font-semibold transition-opacity hover:opacity-90"
//               style={{ backgroundColor: 'var(--primary)', color: 'var(--primary-foreground)' }}
//             >
//               {get('cta_incubator', "Rejoindre l'incubateur")}
//               <ArrowRight className="h-5 w-5" />
//             </a>
//             <Link
//               to="#about"
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16 lg:pt-20">
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold leading-tight"
            style={{ color: 'var(--foreground)' }}>
            {get('hero_title', t('home_hero_title'))}
          </h1>

          <p className="mt-6 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed"
            style={{ color: 'var(--muted-foreground)' }}>
            {get('hero_subtitle', t('home_hero_subtitle'))}
          </p>

          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <a
              href={`https://wa.me/${waNumber}?text=${encodeURIComponent(t('home_cta_incubator'))}`}
              target="_blank"
              rel="noopener noreferrer"
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-4 rounded-lg text-base font-semibold transition-opacity hover:opacity-90"
              style={{ backgroundColor: 'var(--primary)', color: 'var(--primary-foreground)' }}
            >
              {get('cta_incubator', t('home_cta_incubator'))}
              <ArrowRight className="h-5 w-5" />
            </a>
            <Link
              to="#about"
          ...
          <div className="mt-16 grid grid-cols-3 gap-8 max-w-lg mx-auto">
            <div className="text-center">
              <div className="text-2xl md:text-3xl font-bold" style={{ color: 'var(--primary)' }}>50+</div>
              <div className="text-sm mt-1" style={{ color: 'var(--muted-foreground)' }}>{t('home_stats_startups')}</div>
            </div>
            <div className="text-center">
              <div className="text-2xl md:text-3xl font-bold" style={{ color: 'var(--primary)' }}>200+</div>
              <div className="text-sm mt-1" style={{ color: 'var(--muted-foreground)' }}>{t('home_stats_entrepreneurs')}</div>
            </div>
            <div className="text-center">
              <div className="text-2xl md:text-3xl font-bold" style={{ color: 'var(--primary)' }}>15+</div>
              <div className="text-sm mt-1" style={{ color: 'var(--muted-foreground)' }}>{t('home_stats_investors')}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2"
        style={{ color: 'var(--muted-foreground)' }}>
        <span className="text-xs uppercase tracking-widest">{t('home_scroll')}</span>
        <div className="w-px h-8" style={{ background: 'linear-gradient(to bottom, var(--primary), transparent)' }} />
      </div>
    </section>
  );
