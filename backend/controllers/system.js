...
export const deleteNewsletterSubscriber = asyncHandler(async (req, res) => {
  ...
  // Vérifier que l'abonné existe — via le MODEL
  const { rows: existing } = await NewsletterModel.findById(id);
  if (!existing.length) {
    throw new AppError('Abonné introuvable.', 404);
  }

  // Supprimer — via le MODEL
  const { rows } = await NewsletterModel.delete(id);
...
// ── Désinscrire un abonné (soft delete)
export const unsubscribeNewsletter = asyncHandler(async (req, res) => {
  ...
  // Désinscrire — via le MODEL
  const { rows } = await NewsletterModel.unsubscribe(id);
